...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors.map( { case (b1,m) => (b1, m :: history) }).toStream
    //for {
    //  (b1, m) <- b.legalNeighbors.toStream
    //} yield (b1, m :: history)
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    initial match {
      ...
      case (block, moveList) #:: tail =>
        ...
        val newMoves = newNeighborsOnly(allNeighbors, explored)
        val newExplored = explored + block
        newMoves ++ from(tail ++ newMoves, newExplored)
    }
